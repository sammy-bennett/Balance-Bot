...
pinMode(rightMtr_B, OUTPUT);
  ...
  if(!mpu.begin()){
    Serial.println("Failed to start MPU sensor!\n");
    while(1){
      Serial.println("MPU FAILED TRY AGAIN");
    };
  }
...
void loop() {
  ...
  if((cur_time_sinceboot-prev_time) >= 500){
...
void set_motors(int leftspeed, int rightspeed){
  ...
  if(leftspeed > 0 ){
    analogWrite(leftMtr_A, leftspeed);
    digitalWrite(leftMtr_B,LOW);
  }
  else if(leftspeed < 0){
    analogWrite(leftMtr_B, -leftspeed); //positive spee in oppasite pin to go the oher way. 
  ...
  if(rightspeed > 0 ){
    analogWrite(rightMtr_A, rightspeed);
    digitalWrite(rightMtr_B,LOW);
  }
  else if(rightspeed < 0){
    analogWrite(rightMtr_B, -rightspeed); //positive spee in oppasite pin to go the oher way. 
    digitalWrite(rightMtr_A,LOW);
  }
